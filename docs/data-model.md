...
评论完整性按结构和平台汇总数分别校验。一级评论声明的回复数、分页结束状态及父子关系保持严格校验；抖音汇总总数可能因隐藏、审核、删除、缓存或采集期间变化而短暂偏离，因此平台总数与最终唯一记录数（或平台采用一级评论口径时的一级评论数）的差异不超过平台总数的 5% 时视为可接受。超过 5% 且分页完整时标记为 `partial`；分页未完成或存在阻断性校验错误时仍标记为 `blocked`，能够安全隔离的不可见父回复等情况仍按 `partial` 处理。
